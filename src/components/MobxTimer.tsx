import { makeAutoObservable, observable, computed, action, autorun, makeObservable } from 'mobx';
...
const SetVal: React.FC<IWithStore1> = observer(({ store }) => {
	...
	return (
		<>
			<button onClick={addSubClick(store, 'num', Number(minusStr))}>{minusStr}</button>
			<button onClick={addSubClick(store, 'num', Number(plusStr))}>{plusStr}</button>
		</>
	);
});
